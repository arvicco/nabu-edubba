---
title: "10 · In those days"
short_title: "10 · In those days"
description: >-
  Breath, arm, and the syllable ri — and the literary register: how
  hymns and epics open with "in those distant days" and seal their
  praise with the word zid, "true."
layout: chapter
course: cuneiform-102
chapter: 10
permalink: /cuneiform/102/10-in-those-days/
course_url: /cuneiform/102/
course_title: "Cuneiform 102"
teaches: ["𒍣", "𒀉", "𒊑"]
shows: []
---

# In those days

Last chapter's proverbs came from the same clay as a receipt: a
household voice, a wish, a complaint any clerk could write between
two tallies. This chapter changes rooms. Hymns and epics are not
written by clerks keeping count; they are composed to be spoken, to
a god or about a king, and Sumerian marks that shift with its own
habits of opening, praising, and repeating. Batch eleven gives you
the vocabulary the shift runs on.

## Batch eleven

<table class="sign-table">
  <thead>
    <tr><th>Sign</th><th>Name</th><th>Reads</th><th>Means</th><th>Notes</th></tr>
  </thead>
  <tbody>
    <tr>
      <td class="script sign-cell">𒍣</td><td>ZI</td><td><em>zi</em></td>
      <td>breath, life; true, righteous</td>
      <td>the adjective behind every "true shepherd" and "true house"
          in the hymns; literary texts often spell the fuller shape
          <em>zid</em>, the form you will meet in every reading below</td>
    </tr>
    <tr>
      <td class="script sign-cell">𒀉</td><td>A₂</td><td><em>a₂</em></td>
      <td>arm; strength; side</td>
      <td>pairs with this batch's other new sign in <em>a₂ zid-da</em>,
          "the right side" — literally "the true/right arm"</td>
    </tr>
    <tr>
      <td class="script sign-cell">𒊑</td><td>RI</td><td><em>ri</em></td>
      <td>the syllable ri (also re); distant</td>
      <td>the sign behind Sumerian epic's favorite way to begin:
          <em>ud ri-a</em> / <em>ud re-a</em>, "in those distant
          days"</td>
    </tr>
  </tbody>
</table>

Five times each, aloud as you write: *zi*, *a₂*, *ri*. Small batch,
large job: between them these three signs open more hymns and close
more praise-lines than any other three in the literary inventory.

## Grammar bite — a register that addresses, not counts

A receipt names a person, a quantity, a witness, and stops — it
counts. A hymn opens by reaching backward in time and then speaks
*to* someone: a god, a king, a temple. Sumerian marks the difference
partly in what it says and partly in fixed phrases that a receipt
never needs. The clearest of them is the formula that starts a
composition rather than a sentence — Sumerian's own way of saying
"once upon a time":

<figure class="reading reading--script">
  <div class="reading-lines">
    <div class="reading-line"><span class="script">𒌓𒊑𒀀</span><span class="translit">ud re-a</span><span class="gloss">"in that distant day" — ud, "day" (101, veteran) + re, "that," this chapter's RI sign carrying a demonstrative sense of distance + the locative -a of chapter 01, carrying the "in"</span></div>
  </div>
  <figcaption class="citation">
    An Old Babylonian catalogue of Sumerian compositions, which
    files a whole work under this opening formula — the librarians'
    habit chapter 07 met at Yale.
    <code>urn:nabu:etcsl:0.2.01:7</code> · license: ETCSL ·
    non-commercial.
  </figcaption>
</figure>

*Ud*, "day," you have owned since 101; this chapter's RI sign, read
*re* here rather than *ri*, supplies the "that" — pointing not at
today but at a day pushed off into the distance, the way a
storyteller's "once" pushes its story out of the present. The last
sign is the quietest: the locative *-a* from chapter 01's tag list,
carrying the "in" — day, distant, *in*. A hymn
does not announce when it happens; it announces that it happened
*then*, in days already far away, and lets that distance carry the
weight.

The other mark of the register is a fixed praise-word riding on a
noun: *en zid*, "true lord"; *e₂ zid*, "true house." Ordinary prose
does not stack "true" onto every lord and every house it mentions;
literary Sumerian does, routinely, and the sense drifts from
"genuine" toward something closer to "worthy of the name" — a seal
of approval stamped by the word *zid* itself.[^limit]

## True son, true arm

The plainest use of *zi(d)* is not in a hymn at all — it is stitched
into names, the way chapter 06 taught you names carry sentences:

<figure class="reading reading--script">
  <div class="reading-lines">
    <div class="reading-line"><span class="script">𒌉𒍣</span><span class="translit">dumu-zi</span><span class="gloss">the name Dumuzi, "the true son" (approximate) — the god of the Inana-and-Dumuzi cycle, already carrying this chapter's word for "true" on an Early Dynastic tablet</span></div>
  </div>
  <figcaption class="citation">
    Early Dynastic name list, ca. 2600–2500 BCE.
    <a href="https://cdli.earth/artifacts/10019">CDLI P010019</a> via
    Nabu, <code>urn:nabu:cdli:p010019:reverse:3:3'</code> · license:
    attribution. Name translations are approximate by nature.
  </figcaption>
</figure>

*Dumu*, "son," is 101's TUR sign in its usual reading, a veteran
here; *zi* is new. A god's name and an ordinary person's name are
built from the identical two signs — the tablet does not tell you
which came first, the name or the divinity.

This chapter's other new sign joins *zid* inside one more attested
name, both new signs sharing a single word:

<figure class="reading reading--script">
  <div class="reading-lines">
    <div class="reading-line"><span class="script">𒈗𒀉𒍣𒁕</span><span class="translit">lugal-a₂-zi-da</span><span class="gloss">"the king is my right arm" (approximate) — lugal, "king" (veteran) + a₂-zi-da, "the true/right arm"</span></div>
  </div>
  <figcaption class="citation">
    Early Dynastic name list, ca. 2600–2500 BCE.
    <a href="https://cdli.earth/artifacts/10063">CDLI P010063</a> via
    Nabu, <code>urn:nabu:cdli:p010063:reverse:6:5</code> · license:
    attribution. Name translations are approximate by nature.
  </figcaption>
</figure>

*A₂ zid-da*, "the right side," is literally "the true arm" — and
the *-da* is chapter 03's sign doing spelling-glue work, writing
out *zid*'s own final /d/ with a vowel to carry it, exactly the
job *-la-* did inside chapter 05's formula. A single name, both of
this chapter's new signs, no gaps.

## Lady of the true powers

Move from names to the hymns proper, and *zid* stops decorating a
person and starts decorating a god:

<figure class="reading reading--script">
  <div class="reading-lines">
    <div class="reading-line"><span class="script">𒎏𒈨𒍣𒁕</span><span class="translit">nin me zid-da</span><span class="gloss">"lady of the true me" — nin, "lady" (ch. 05, veteran) + me, "the divine powers" (ch. 00, veteran) + zid-da, "true"</span></div>
  </div>
  <figcaption class="citation">
    The same Old Babylonian catalogue, sixteen entries later — a
    goddess's hymn filed under its own first words.
    <code>urn:nabu:etcsl:0.2.01:23</code>
    · license: ETCSL · non-commercial.
  </figcaption>
</figure>

The catalogue that filed a composition under *ud re-a* files
another under this: a hymn to a lady — not merely the lady who
holds the *me*, chapter 00's word for the divine powers that keep
the world running, but the lady of the *true* ones. Praise in this
register rarely stops at naming what someone holds; it certifies
the holding as genuine.

## Poured out by the great mountain

*Zid* and *ri* meet in a single praise-line from the same temple
hymn that gave last chapter's "its heart is unknowable":

<figure class="reading reading--script">
  <div class="reading-lines">
    <div class="reading-line"><span class="script">𒂗𒍣𒀀𒆳𒃲𒂊𒊑𒀀</span><span class="translit">en zid a kur gal-e ri-a</span><span class="gloss">"true lord, seed poured out by the great mountain" — en zid, "true lord" + a … ri-a, "engendered, poured out" + kur gal-e, "by the great mountain," Enlil's own praise-name, marked with the doer's tag -e from chapter 01</span></div>
  </div>
  <figcaption class="citation">
    Gudea's temple hymn. <code>urn:nabu:etcsl:2.1.7:210</code> ·
    license: ETCSL · non-commercial.
  </figcaption>
</figure>

Read it in order: the temple's own god is hailed as *en zid*,
"true lord," then credited to a divine source — *a … ri-a* frames
him as water "poured out," engendered, and the source is *kur
gal-e*, "the great mountain," one of the god Enlil's own
praise-names, wearing the doer's tag *-e* you first met on
*lugal-e* back in chapter 01. Four
of this chapter's readings stack the same word, *zid*, over four
different kinds of nobility — a son, an arm, a set of powers, a
lord — and every time it means the same thing: this one is the real
one.

## A house says it twice

Praise is not the register's only mode. When a temple falls, the
same *zid* returns inside a different kind of line — one that says
its point by saying it again:

<figure class="reading reading--script">
  <div class="reading-lines">
    <div class="reading-line"><span class="script">𒀀𒂍𒍣𒂍𒍣𒀀𒇽𒁉𒇽𒁉</span><span class="translit">a e₂ zid e₂ zid a lu₂-bi lu₂-bi</span><span class="gloss">"alas, true house! true house, alas! its people, its people" — a lament's opening cry, the true house named twice, its people named twice</span></div>
  </div>
  <figcaption class="citation">
    City lament. <code>urn:nabu:etcsl:2.2.3:F.358</code> · license:
    ETCSL · non-commercial.
  </figcaption>
</figure>

*A* opens the line — the same water-sign you have owned since 101's
first chapter, wearing yet another job here: not "water" but a bare
cry of grief, "alas." *E₂ zid*, "true house," is 101's *e₂*
carrying this chapter's praise-word; *lu₂-bi*, "its people," is
101's *lu₂* with chapter 01's *-bi*. Nothing in the line is new
grammar — what is new is the habit of saying the same true thing
twice before moving on, so that the loss lands twice as hard. A
receipt never repeats a line for effect; a lament always does.

## Drill before you go on

1. Write 𒍣, 𒀉, 𒊑 from memory; say *zi*, *a₂*, *ri*.
2. Read *ud re-a* aloud and explain, in one sentence, what job the
   "distant" sense of *ri/re* is doing there.
3. Name the four different nouns that *zid* attaches to across this
   chapter's readings, and say in one sentence what the word adds
   each time.
4. Write *a₂-zid-da* and *e₂ zid* from memory, sign by sign, and say
   which chapters taught every sign in each.
5. Explain why a lament repeats *e₂ zid* and *lu₂-bi* instead of
   saying each once.

*Next: a real dedication, read whole from actual stone — and the
stretch's close.*

[^limit]: An honest limit: the full literary language carries far
    more machinery than this — a whole second dialect, *emesal*,
    used by certain speakers and by laments, and tiers of rare
    signs no receipt ever needed. This course leaves all of that
    for the standard grammars to teach properly.
