---
title: "07 · I am king"
short_title: "07 · I am king"
description: >-
  The enclitic copula -me-en: how Sumerian says "I am X" with signs
  you already own — plus batch eight, the royal vocabulary that gives
  it something to say.
layout: chapter
course: cuneiform-102
chapter: 7
permalink: /cuneiform/102/07-i-am-king/
course_url: /cuneiform/102/
course_title: "Cuneiform 102"
teaches: ["𒆬", "𒈤", "𒌧"]
shows: []
---

# I am king

Chapter 06 closed with a promise: the little word that lets a king
say "I am" — and the palace-sized vocabulary that follows it. Here
is the surprise this chapter opens with: the little word is not new.
You have owned both of its signs since 101 and since chapter 00 of
this very course. What is new is only the vocabulary a king reaches
for once he can say it.

## Batch eight

<table class="sign-table">
  <thead>
    <tr><th>Sign</th><th>Name</th><th>Reads</th><th>Means</th><th>Notes</th></tr>
  </thead>
  <tbody>
    <tr>
      <td class="script sign-cell">𒆬</td><td>KU₃</td><td><em>ku₃</em></td>
      <td>pure, holy; precious metal (silver)</td>
      <td>~5 wedges; silver is "pure metal" in Sumerian long before it
          is anything else — and royal praise leans on this word
          constantly. Its fuller spoken shape is <em>kug</em>, the
          final consonant surfacing before vowels the same way
          chapter 02's hidden genitive does</td>
    </tr>
    <tr>
      <td class="script sign-cell">𒈤</td><td>MAH</td><td><em>mah</em></td>
      <td>great, exalted</td>
      <td>~7 wedges; the majesty word — combine it with chapter 03's
          abstraction-maker and you get <em>nam-mah</em>,
          "greatness, majesty," one of the commonest words in royal
          titulary</td>
    </tr>
    <tr>
      <td class="script sign-cell">𒌧</td><td>KALAM</td><td><em>kalam</em></td>
      <td>the Land — Sumer, as its own texts name itself</td>
      <td>~9 wedges; not "a land" among others but <em>the</em> Land,
          the way a Sumerian text names its own country — kings are
          <em>lugal kalam-ma</em>, "king of the Land"</td>
    </tr>
  </tbody>
</table>

Five times each, aloud as you write: *ku₃*, *mah*, *kalam*. That is
the eighth batch of this course, and this one is not idle vocabulary
— every sign in it goes to work before this chapter ends.

## Grammar bite — the little word for "to be"

Sumerian builds "I am X" by gluing a short tag onto the end of X:
**-me-en**. Attach it to *lugal*, "king," and you get *lugal-me-en*,
"I am (the) king." Now look closely at what that tag is made of:

<figure class="composition">
  <p class="composition-signs" style="font-size: 2.6em; letter-spacing: 0.2em; margin: 0.4em 0 0.2em;">𒈗𒈨𒂗</p>
  <p class="composition-translit" style="opacity: 0.85; margin: 0 0 0.6em;">lugal- me -en</p>
  <figcaption class="citation">
    Assembled from your own inventory, sign by sign: <strong>𒈗</strong>
    <em>lugal</em> ("king," 101 ch. 06) + <strong>𒈨</strong> <em>me</em>
    (this course's very first sign, ch. 00) + <strong>𒂗</strong>
    <em>en</em> (101 ch. 10) — "I am king." Zero new signs. This
    chapter's batch is not what says it; it is what royalty says
    with it.
  </figcaption>
</figure>

*Me* and *en* are both veterans doing new work, not new entries —
you met *me* as "rites, divine powers" and *en* as "lord, high
priest," and neither of those meanings has gone anywhere. Fused
together as a suffix, the same two signs spell the copula: not a
noun and a title standing side by side, but the verb "to be" itself,
riding on the end of whatever it names. This is exactly how it
appears in a real hymn's opening line:

<figure class="reading reading--script">
  <div class="reading-lines">
    <div class="reading-line"><span class="script">𒈗𒈨𒂗</span><span class="translit">lugal-me-en</span><span class="gloss">"I am the king" — the incipit line by which an Old Babylonian library catalogue identifies a royal hymn, the kind that opens by having the king speak in his own voice</span></div>
  </div>
  <figcaption class="citation">
    Y2, an Old Babylonian catalogue tablet at Yale listing the
    opening lines of Sumerian literary compositions. Entry A.1.
    <code>urn:nabu:etcsl:0.2.12:A.1</code> · license: ETCSL ·
    non-commercial.
  </figcaption>
</figure>

A librarian, thousands of years ago, filed a whole hymn under its
first three signs — because the hymn itself opens with the king
announcing exactly what those signs say. Many Sumerian royal hymns
work this way: the king does not get introduced in the third person;
he speaks, and the first thing out of his mouth is *lugal-me-en*.

One more thing, worth a single sentence and no table: Sumerian also
has a third-person copula, "it is" or "he/she/it is," which shows up
as *-am₃* — commonly written with two more old friends of yours,
𒀀 and 𒀭. You are not learning that form yet; just notice it exists,
so it does not surprise you later.

## Vocabulary that leans on it

Now the batch goes to work. *Nam-mah*, "majesty," is chapter 03's
abstraction-maker in front of this chapter's new majesty word — the
same production line that turned *lugal* into *nam-lugal*,
"kingship":

<figure class="reading reading--script">
  <div class="reading-lines">
    <div class="reading-line"><span class="script">𒉆𒈤</span><span class="translit">nam-mah</span><span class="gloss">"majesty, greatness" — nam- (abstraction, ch. 03) + mah, this chapter's new sign for "great, exalted"</span></div>
  </div>
  <figcaption class="citation">
    <a href="https://cdli.earth/artifacts/10059">CDLI P010059</a>
    via Nabu, <code>urn:nabu:cdli:p010059:reverse:3:10</code> ·
    license: attribution.
  </figcaption>
</figure>

And *ku₃* alone, in a phrase that shows why "pure" and "silver" are,
in practice, one word:

<figure class="reading reading--script">
  <div class="reading-lines">
    <div class="reading-line"><span class="script">𒂍𒆬</span><span class="translit">e₂ ku₃</span><span class="gloss">"the pure house" — a temple's stock epithet, house + pure</span></div>
  </div>
  <figcaption class="citation">
    <a href="https://cdli.earth/artifacts/10644">CDLI P010644</a>
    via Nabu, <code>urn:nabu:cdli:p010644:obverse:3:2</code> ·
    license: attribution.
  </figcaption>
</figure>

A temple is not just a house; it is the *pure* house — the same
word that, applied to metal, means silver. One sign, one idea,
carried into two different rooms.

## The genitive's hidden consonant, caught in the open

Chapter 02 taught you the genitive *-ak* and warned you it likes to
hide: *e₂ lugal-ak*, "the king's house," routinely surfaces on clay as
*e₂ lugal-a*, its final /k/ swallowed. That chapter promised the /k/
comes back the moment a vowel gives it somewhere to land. Here is
that promise, kept, in *kalam*, this chapter's own new sign:

<figure class="reading reading--script">
  <div class="reading-lines">
    <div class="reading-line"><span class="script">𒌧𒈠𒅗</span><span class="translit">kalam-ma-ka</span><span class="gloss">"of the Land" — kalam + the genitive -ak, its hidden /k/ surfacing before a further vowel, written out in full as -ka</span></div>
  </div>
  <figcaption class="citation">
    From a praise poem of Enlil-bani. <code>urn:nabu:etcsl:2.5.8.1:181</code>
    · license: ETCSL · non-commercial.
  </figcaption>
</figure>

Read the chain slowly: *kalam* ("the Land"), then *-ma-* — the
word's own final /m/ written again as glue, the same job chapter
05's *-la-* did for *til* — carrying the genitive's vowel, and then
the genitive's /k/ stepping fully into view in *-ka* because, this
time, a vowel follows for it to stand on. This is the exact
mechanism chapter 02 described in the abstract, now sitting in
front of you written out: the tag was never gone, only shy.

<figure class="reading reading--script">
  <div class="reading-lines">
    <div class="reading-line"><span class="script">𒎏𒍪𒀀𒍪𒃲𒌧𒈠</span><span class="translit">nin-zu a-zu gal kalam-ma</span><span class="gloss">"your lady, the great healer of the Land" — lady + your + water-knower (healer) + great + the Land (-genitive, hidden)</span></div>
  </div>
  <figcaption class="citation">
    From <em>The temple hymns</em>. <code>urn:nabu:etcsl:4.80.1:392</code>
    · license: ETCSL · non-commercial.
  </figcaption>
</figure>

Here the same *kalam-ma* sits at the end of the line, /k/ hidden
again, exactly as chapter 02 predicted it would — one phrase catching
the consonant out in the open, another catching it hiding, and now
you can tell the difference on sight.

One more thing rode into that line and deserves to be named: the
*-zu* on *nin-zu* is
[chapter 03's]({{ '/cuneiform/102/03-say-it-twice/' | relative_url }})
knowing-sign 𒍪 in a second job — as a suffix it means "your."
You already own its partner: the *-ni*, "his/her," inside
[chapter 05's]({{ '/cuneiform/102/05-a-dedication-whole/' | relative_url }})
*nam-ti-la-ni-še₃*. *-zu* is the "your" of that same family, and
the register this chapter opened leans on it constantly — praise
is always spoken *to* someone. A veteran sign, a new reading;
the verb "to know" has not gone anywhere.

## Drill before you go on

1. Write 𒆬, 𒈤, 𒌧 from memory; say *ku₃*, *mah*, *kalam*.
2. Write *lugal-me-en* from memory, sign by sign, and say what each
   of its three signs meant before it joined this phrase.
3. Build *nam-mah* and *e₂ ku₃* in signs, and give both meanings each
   carries.
4. Read *kalam-ma-ka* aloud and explain, in one sentence, why its /k/
   is written out while the /k/ in *e₂ lugal-ak* usually is not.
5. Say aloud what *-am₃* is a preview of, and which two signs
   commonly spell it.

*Next: the verbal chain taken apart — the prefixes ba-, i₃-, and
in-, and how Sumerian says no.*
