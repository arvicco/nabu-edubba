---
title: "KI · place"
description: >-
  The veteran sign KI in Akkadian — a plain syllable in
  ki-nu-ni, while its old Sumerian self opens the contract
  formulary's "from."
layout: sign
school: cuneiform
sign: "KI"
reads: "[ki]"
course: cuneiform-addenda-akk
course_url: /cuneiform/addenda-akk/
course_title: "Akkadian Addenda"
permalink: /cuneiform/addenda-akk/signs/ki/
---

## Where it comes from

<a href="{{ '/cuneiform/101/12-reference/#sign-121A0' | relative_url }}">101's place-sign</a> —
earth and ground, the sign under every city name. In Akkadian
text it works twice over: as a plain syllable
(<em>ki-nu-ni</em>, "of the brazier-festival"), and — in the
contracts' Sumerian formulary — as its old self, opening
"from (the hands of) so-and-so."

## How to remember it

**Place** twice over: the ground a festival stands on when it
is a syllable, and the place a loan comes FROM when the
notary's old Sumerian does the talking.

## In the wild

<figure class="reading reading--script">
  <div class="reading-lines">
    <div class="reading-line"><span class="script"><span class="logo">𒌓</span> 𒆠𒉡𒉌</span><span class="translit"><span class="logo">ŪM</span> ki-nu-ni</span><span class="gloss"><span class="norm">ūm kinūni</span>, "on the day of the brazier-festival" — a loan's due date</span></div>
  </div>
  <figcaption class="citation">
    An Old Babylonian loan (PBS 8/2, 195), obverse 9.
    <code>urn:nabu:cdli:p257793:obverse:9</code> · license:
    attribution.
  </figcaption>
</figure>

Akkadian reading taught in
<a href="{{ '/cuneiform/103/17-silver-changes-hands/' | relative_url }}">103's chapter 17</a>.
