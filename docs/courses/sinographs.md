...
- **The standard voice (owner ruling 2026-08-15; supersedes the
  acquisition law above and its never-TTS bar):** all course audio
  — syllable citations AND classical reading lines — is
  SYNTHESIZED in one pinned voice (vendor: ElevenLabs; model and
  voice recorded in assets-src/data/voice-plan.yml `engine:` and
  in the ledger of every built clip, voice-ledger.yml). Rationale,
  owner's words: no one can pronounce Old Chinese "natively"
  anyways — every modern voicing of a classical text is a
  convention, and one consistent voice teaches better than a
  patchwork of borrowed human clips (the 2026-08-15 unevenness
  report). What does NOT change: every syllable clip is still
  pitch-verified against its declared tone and span-checked, every
  clip is loudness-normalized (−20/−1), filenames carry no tone
  digits, and the say-audio lint holds the surface. New law
  besides: (a) the credits files state plainly that the audio is
  synthesized, naming engine and voice; (b) synthesis runs are
  OWNER-run only — `rake voice:synth` with ELEVENLABS_API_KEY in
  the environment; the key never enters the repo and the agent
  never runs the synth task (golden rules 3 and 8) — the agent
  prepares batches (`rake voice:batch`) and QA-integrates results
  (`rake voice:integrate`); (c) a clip that fails QA does not
  ship and returns in the next batch; (c′) the LENGTH LAW (owner
  review 2026-08-15: fluent-pace syllables are useless — "a
  novice just can't hear the tone"): a syllable citation clip
  must hold a voiced span inside the plan's syllable_span
  (currently 0.4 to 2.2 s) — spoken at the deliberate
  syllable_voice_settings pace —
  or the gate refuses it; length is a shipping requirement, not a
  style preference; (d) reading lines are
  voiced whole (modern Mandarin, natural sandhi) into
  site/assets/audio/lines/, duration-checked, with a
  syllable-count report; the pinyin line of a reading becomes the
  say-link, same as sign tables. The old Commons pipeline
  (bin/pinyin_audio.rb, pinyin-audio-sources.yml) retires when the
  first full regeneration lands, kept in-tree as the QA library
  and the record of the human-clip era.
- **Ear-approval — the final referee (owner workflow 2026-08-16):**
  the machine gates (pitch, span, transcript) are individually
  noisy on half-second clips; they ADVISE, the owner's ear RULES.
  A take the gates refused may ship only by the owner naming it
  from the review page (`bin/pinyin_voice.rb approve <take-id>`);
  it ships loudness-normalized like any other clip and is ledgered
  `ear_approved: true` with the winning take id — the ledger
  always shows which clips the machine passed and which the owner
  overrode.
- **The contrast-row law (owner review 2026-08-16):** an audio
  demo row varies exactly ONE thing. A row that teaches a
  consonant contrast holds the tone (and where possible the vowel)
  constant across its clips — pà·tà·kǒu hides the aspiration
  behind three different tones; bù·dù·gù against pù·tù·kù exposes
  it, column by column, as minimal pairs. Same principle for vowel
  rows: hold the tone, vary the vowel. Demo syllables are chosen
  (and synthesized) to make the uniform sets possible; a mixed-tone
  contrast row is a defect, not a style choice.
...
| One standard synthesized voice, pinned + ledgered (2026-08-15; supersedes CC-only/never-TTS) | §2 | voice-plan `engine:` pin + voice-ledger.yml |
| Synthesis is owner-run; the key never enters the repo | §2 | rake voice:synth env guard (refusal without ELEVENLABS_API_KEY) |
| Audio labeled as synthesized, engine and voice named | §2 | CREDITS files regenerated by voice:integrate |
...
| Citation clips long enough to hear the tone (syllable_span, 0.4–2.2 s) | §2 | pinyin_voice integrate span gate (syllable_span) |
...
| Machine gates advise, the owner's ear rules (ear-approval) | §2 | approve subcommand + ledger `ear_approved:` |
| A demo row varies exactly one thing (contrast-row law) | §2 | owner review (editorial) |
