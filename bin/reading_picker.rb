...
module Edubba
  module ReadingPicker
    ...
    # Reinforcement score (rulebook cuneiform.md §8, owner ruling
    # 2026-08-04): how many distinct older signs due for review a
    # candidate line ALSO exercises — values introduced 1, 2, 4, or
    # 8 chapters before the line's bucket. Sweeps sort by this, so
    # a line that revises while it teaches outranks one that only
    # shows the new sign.
    VINTAGES = [1, 2, 4, 8].freeze

    def reinforcement(tokens, chapter, value_chapter)
      hits = tokens.uniq.select do |t|
        seat = value_chapter[t]
        seat && VINTAGES.include?(chapter - seat)
      end
      [hits.size, hits]
    end
  end
...
if $PROGRAM_NAME == __FILE__
  ...
  # value -> introducing queue chapter, per corpus convention, for
  # the reinforcement score (§8).
  val2ch = { "cdli" => {}, "etcsl" => {} }
  queue.select { |s| s["chapter"] }.each do |s|
    val2ch["cdli"][s["value"]] = s["chapter"]
    val2ch["etcsl"][s["etcsl_value"] || Edubba::ReadingPicker.fold_etcsl(s["value"])] = s["chapter"]
  end
  ...
  SOURCES.each_key do |source|
    ...
    IO.popen([NABU, "export", "--format=jsonl", "--lang=sux", "--source=#{source}"],
      ...
      io.each_line do |line|
        ...
        score, hits = Edubba::ReadingPicker.reinforcement(tokens, ch, val2ch[source])
        buckets[[ch, source]] << [score, "#{rec['urn']}  |  #{rec['text']}  |  #{glosses.join(' / ')}  |  reinforces: #{score}#{hits.empty? ? '' : " (#{hits.join(' ')})"}"]
        kept += 1
      end
    end
