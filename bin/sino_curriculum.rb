...
OUT102 = "site/_data/sinographs102_queue.yml"
...
module Edubba
  module SinoCurriculum
    ...
    def validate!(pool, freq)
      # chapters are per-course ordinals since the border split
      # (concept §7): S101 rows carry no course key, S102 rows say
      # course: 102 and count 1-15
      pool.group_by { |s| [s["course"] || 101, s["chapter"]] }.each do |(course, ch), signs|
        abort "sino_curriculum: unpinned row #{signs.map { |s| s['char'] }.join(',')}" if ch.nil?
        next if FRESH.cover?(signs.size)

        abort "sino_curriculum: S#{course} ch #{ch} pins #{signs.size} characters — the law says " \
...
if $PROGRAM_NAME == __FILE__
  ...
  # The course border (owner ruling 2026-08-12, concept §7): S101
  # is closed at chapter 09; everything beyond belongs to S102.
  # One pool, two registries — the border is enforced at emit time.
  { OUT => signs.reject { |s| s["course"] == 102 },
    OUT102 => signs.select { |s| s["course"] == 102 } }.each do |out, course_signs|
    course = out[/sinographs\d+/]
    running = 0
    course_batches = course_signs.group_by { |s| s["chapter"] }.sort.map do |ch, batch|
    ...
    header = <<~HEADER
      ...
      # + char-freq-kanripo.tsv — do not edit by hand. #{course.sub('sinographs', 'S')}
      # character queue: chapter = the teaching chapter. The course
      # border (concept §7, 2026-08-12): S101 holds chapters 00-09
      # only; S102 the rest. Contract: additive changes only once
      # published (CLAUDE.md rule 5).
    HEADER
    doc = {
      ...
      "signs" => course_signs, "batches" => course_batches
    }
    File.write(out, header + doc.to_yaml.sub(/\A---\n/, ""))
    puts "sino_curriculum: #{course_signs.size} characters, " \
         "#{course_batches.size} batches -> #{out}"
  end
end
