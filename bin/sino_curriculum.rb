...
if $PROGRAM_NAME == __FILE__
  ...
  { OUT => signs.select { |s| s["chapter"] <= 9 },
    OUT102 => signs.select { |s| s["chapter"] > 9 } }.each do |out, course_signs|
    course = out[/sinographs\d+/]
    course_batches = batches.select do |b|
      course == "sinographs101" ? b["chapter"] <= 9 : b["chapter"] > 9
    end
    header = <<~HEADER
